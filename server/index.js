...
app.all('*', function(req, res, next){
    ...
    res.header("Access-Control-All-Header",'*');
    res.header("Access-Control-All-Methods","PUT,POST,GET,DELETE,OPTIONS");
...
function sendSMS (phone){
    ...
    return smsClient.sendSMS({
        ...
        TemplateCode: 'SMS_14457028',//必填:短信模板-可在短信控制台中找到，发送国际/港澳台消息时，请使用国际/港澳台短信模版
        TemplateParam: `{"code":${Math.floor(Math.random()*899999)+100000}}`//可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"时。
    }).then(function (res) {
        let {Code}=res
        if (Code === 'OK') {
            //处理返回参数
